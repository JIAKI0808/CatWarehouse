 ...
 * 写操作工厂（创建型）。
 *
 * 改造前 `create` / `update` / `delete` 在 10 个资源里逐字重复了 **30 遍**，
 * 差异只有路径和 TS 类型 —— 这是本文件最大的一处重复。
 *
 * 为什么**不带读操作**：列表与单条读取在 10 个资源里有 6 种形态
 * （`getAll()` / `getAll(params)` / `getByCategory(id)` / `getBySubCategory(id, q)` /
 * `search(q)` / `getQuantity(id)`），且每条都只有一行。把它们也塞进工厂，
 * 只会多出几个没有使用者的旋钮，换来的是一个没人要的「统一读取」。
 *
 * 每个资源用展开运算符合入，**键的顺序与原实现一致**（读在前、写在中间、自定义在后）。
 */
...
export const subCategoryApi = {
  ...
  ...createWriteApi<SubCategory, SubCategoryCreate, SubCategoryUpdate>(
    '/api/sub-categories'
  ),
...
export const recurringApi = {
  ...
  ...createWriteApi<RecurringBill, RecurringBillCreate, RecurringBillUpdate>(
    '/api/recurring'
  ),
  generate: () =>
 ...
 * 手设反而会破坏请求。所以这里走裸 `fetch` + `FormData`，并且**不设** `Content-Type`。
 *
